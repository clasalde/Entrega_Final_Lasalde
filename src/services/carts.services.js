...
// const EmailManager = require("../utils/email.js"); 
...
//     console.log(`Iniciando proceso de compra para carrito: ${cartId}`);
//     const cart = await cartsRepository.getCartById(cartId, false);
...
//         await this.emailManager.enviarCorreoCompra(
//           user.email,
//           user.first_name,
//           ticket.code
//         );
...
class CartsServices {
  ...
  async finishPurchase(cartId) {
    ...
    if (user && user.email) {
      try {
        console.log(`Intentando enviar correo a: ${user.email}`);
        await this.emailManager.enviarCorreoCompra(user.email, user.first_name, ticket.code);
        console.log(`Correo enviado con éxito a: ${user.email}`);
      } catch (error) {
      ...
      console.log(`No se pudo enviar correo. Usuario o email no disponible.`);
    }
